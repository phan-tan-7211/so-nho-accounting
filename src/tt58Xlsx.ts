...
interface XlsxCell { value?: ReportCell; style?: number }
interface XlsxSheet {
  name: string;
  rows: XlsxCell[][];
  merges: string[];
...
export interface Tt58XlsxOptions { draft?: boolean }
...
function c(value?: ReportCell, style = STYLE_NORMAL): XlsxCell { return { value, style }; }
...
function cellText(value: ReportCell | undefined): string { return value === undefined ? '' : String(value); }
...
function commonHeader(
  ...
  return {
    rows: [
      ...
      [c(title, STYLE_TITLE)],
      [c(`Năm: ${new Date(bundle.periodStart).getFullYear()}`, STYLE_CENTER)],
      [c(subtitle, STYLE_CENTER)],
    ],
...
function revenueSheet(bundle: Tt58ReportBundle, table: Tt58ReportTable, draft: boolean): XlsxSheet {
  ...
  for (const row of table.rows) {
    ...
    if (type === 'ENTRY') {
      ...
      const key = `${label}|${row[6]}|${row[7]}`;
      if (key !== currentGroup) {
        currentGroup = key;
        groupIndex += 1;
        const rates = [
          typeof row[6] === 'number' ? `GTGT ${row[6]}%` : '',
          typeof row[7] === 'number' ? `TNDN ${row[7]}%` : '',
        ].filter(Boolean).join(' · ');
    ...
    } else if (type === 'GROUP_TOTAL') {
      rows.push([c(), c(), c(`Tổng cộng nhóm ${cellText(row[4])}`, STYLE_BOLD), money(row[5], true)]);
      if ((table.code === 'S1-DNSN' || table.code === 'S2a-DNSN') && typeof row[6] === 'number') rows.push([c(), c(), c('Thuế GTGT', STYLE_BOLD), money(row[6], true)]);
      if ((table.code === 'S1-DNSN' || table.code === 'S3a-DNSN') && typeof row[7] === 'number') rows.push([c(), c(), c('Thuế TNDN', STYLE_BOLD), money(row[7], true)]);
    } else if (type === 'TOTAL') {
...
function s2bSheet(bundle: Tt58ReportBundle, table: Tt58ReportTable, draft: boolean): XlsxSheet {
  ...
  for (const row of table.rows) {
    ...
    } else if (type === 'TOTAL_REVENUE') rows.push([c(), c(), c('Cộng doanh thu và thu nhập', STYLE_BOLD), money(row[6], true)]);
...
function splitS2cSections(table: Tt58ReportTable): ReportCell[][][] {
  ...
  for (const source of table.rows) {
    const row = [...source];
    if (row[0] === 'OPENING' && current.length > 0) { sections.push(current); current = []; }
    current.push(row);
  }
...
function s2cSheets(bundle: Tt58ReportBundle, table: Tt58ReportTable, draft: boolean): XlsxSheet[] {
  return splitS2cSections(table).map((section, index) => {
    ...
    for (const row of section) {
      const type = cellText(row[0]);
      if (type === 'OPENING') {
        rows.push([c(), c(), c('Tồn đầu kỳ', STYLE_BOLD), c(unit, STYLE_CENTER), money(row[12]), c(), c(), c(), c(), c(parseQuantity(row[10]), STYLE_CENTER), money(row[11], true)]);
      } else if (type === 'ENTRY') {
        const quantity = parseQuantity(row[7]);
        const direction = cellText(row[8]);
        rows.push([
          c(row[5]), c(row[4], STYLE_CENTER), c(row[6]), c(unit, STYLE_CENTER), money(row[12]),
          direction === 'IN' ? c(quantity, STYLE_CENTER) : c(), direction === 'IN' ? money(row[9]) : c(),
          direction === 'OUT' ? c(quantity, STYLE_CENTER) : c(), direction === 'OUT' ? money(row[9]) : c(),
          c(parseQuantity(row[10]), STYLE_CENTER), money(row[11]),
        ]);
      } else if (type === 'TOTAL') {
        rows.push([c(), c(), c('Tồn cuối kỳ', STYLE_BOLD), c(unit, STYLE_CENTER), money(row[12], true), c(), c(), c(), c(), c(parseQuantity(row[10]), STYLE_CENTER), money(row[11], true)]);
      }
    }
    appendFooter(rows, merges, 11, draft);
    return { name: uniqueSheetName(`S2c-${itemCode || index + 1}`, index), rows, merges, widths: [17, 14, 34, 10, 14, 13, 18, 13, 18, 13, 18], landscape: true };
  });
}
...
function s3bSheet(bundle: Tt58ReportBundle, table: Tt58ReportTable, draft: boolean): XlsxSheet {
  ...
  for (const row of table.rows) {
    const type = cellText(row[0]);
    if (type === 'ENTRY') rows.push([c(row[2]), c(row[1], STYLE_CENTER), c(row[3]), money(row[4]), money(row[5])]);
    else if (type === 'TOTAL') rows.push([c(), c(), c('Cộng số phát sinh trong kỳ', STYLE_BOLD), money(row[4], true), money(row[5], true)]);
    ...
    } else if (type === 'TAX_PAID') rows.push([c(), c(), c('Số thuế GTGT đã nộp trong kỳ (3)', STYLE_BOLD), c(), money(row[5], true)]);
    else if (type === 'TAX_REFUNDED') rows.push([c(), c(), c('Số thuế GTGT đã được hoàn trong kỳ (4)', STYLE_BOLD), money(row[5], true), c()]);
...
function tableSheets(bundle: Tt58ReportBundle, table: Tt58ReportTable, draft: boolean): XlsxSheet[] {
  switch (table.code) {
    case 'S1-DNSN':
    case 'S2a-DNSN':
    case 'S3a-DNSN': return [revenueSheet(bundle, table, draft)];
...
function worksheetXml(sheet: XlsxSheet): string {
  ...
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetViews><sheetView workbookViewId="0"><pane ySplit="7" topLeftCell="A8" activePane="bottomLeft" state="frozen"/></sheetView></sheetViews><cols>${cols}</cols><sheetData>${rowsXml}</sheetData>${merges}<pageMargins left="0.3" right="0.3" top="0.5" bottom="0.5" header="0.2" footer="0.2"/><pageSetup orientation="${sheet.landscape ? 'landscape' : 'portrait'}" fitToWidth="1" fitToHeight="0"/></worksheet>`;
}
function stylesXml(): string {
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><numFmts count="1"><numFmt numFmtId="164" formatCode="#,##0;[Red]-#,##0"/></numFmts><fonts count="5"><font><sz val="11"/><name val="Arial"/></font><font><b/><sz val="14"/><name val="Arial"/></font><font><b/><sz val="11"/><name val="Arial"/></font><font><i/><sz val="10"/><name val="Arial"/></font><font><b/><sz val="11"/><name val="Arial"/></font></fonts><fills count="3"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill><fill><patternFill patternType="solid"><fgColor rgb="FFEFEFEF"/><bgColor indexed="64"/></patternFill></fill></fills><borders count="2"><border/><border><left style="thin"/><right style="thin"/><top style="thin"/><bottom style="thin"/></border></borders><cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellStyleXfs><cellXfs count="8"><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"><alignment vertical="top" wrapText="1"/></xf><xf numFmtId="0" fontId="1" fillId="0" borderId="0" xfId="0"><alignment horizontal="center" vertical="center"/></xf><xf numFmtId="0" fontId="2" fillId="2" borderId="1" xfId="0"><alignment horizontal="center" vertical="center" wrapText="1"/></xf><xf numFmtId="0" fontId="2" fillId="0" borderId="0" xfId="0"><alignment vertical="top" wrapText="1"/></xf><xf numFmtId="164" fontId="0" fillId="0" borderId="1" xfId="0"><alignment horizontal="right" vertical="top"/></xf><xf numFmtId="0" fontId="0" fillId="0" borderId="1" xfId="0"><alignment horizontal="center" vertical="top" wrapText="1"/></xf><xf numFmtId="0" fontId="3" fillId="0" borderId="0" xfId="0"><alignment horizontal="center" vertical="center"/></xf><xf numFmtId="164" fontId="4" fillId="0" borderId="1" xfId="0"><alignment horizontal="right" vertical="top"/></xf></cellXfs><cellStyles count="1"><cellStyle name="Normal" xfId="0" builtinId="0"/></cellStyles></styleSheet>`;
}
...
function textEntry(name: string, content: string): ZipEntry { return { name, data: new TextEncoder().encode(content) }; }
